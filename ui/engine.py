...
def load_compliance_rules() -> str:
    """Load content-writing-compliance rules for injection into zhizao/zhiyou prompts."""
    path = STEERING_PATH / "content-writing-compliance.md"
    if path.exists():
        return path.read_text(encoding="utf-8")
    return ""
...
审查要求：若发现敏感词或违规表述，compliance_status 标记为 FIXED，在 fixes_applied 中说明替换了哪些词，在 final_content 中输出替换后的内容。"""
