...
with st.sidebar:
    ...
    _default_lang_idx = 0  # English fallback

    # Detect proper language for user based on their region/sub_region
    _detected_lang_idx = 0
    if _current_user_for_lang:
        try:
        ...
        if _u_sub_region == "TW":
            ...
            _detected_lang_idx = 0  # English (NA/EU/other)

    # On first login (or user switch), force the correct language
    _lang_init_key = f"_lang_init_{_current_user_for_lang}"
    if _current_user_for_lang and _lang_init_key not in st.session_state:
        _default_lang_idx = _detected_lang_idx
        st.session_state[_lang_init_key] = True
        # Remove stale ui_lang so selectbox uses our index
        if "ui_lang" in st.session_state:
            del st.session_state["ui_lang"]
    elif "ui_lang" in st.session_state:
        # User already has a selection — respect it
...
# ============================================================
    ...
    if data:
        ...
        # JSON preview — LEGO Sell Design format only
        st.divider()
        ...
        if items:
            ...
            if not _has_lego:
                st.caption("💡 LEGO Sell Design JSON will be generated when you run 智布")
            ...
            for i, item in enumerate(items):
                ...
                with col_prev:
                    ...
                    _lego_file = _lego_dir / f"{_lego_fname}.json" if _has_lego else None
                    if _lego_file and _lego_file.exists():
                ...
                with col_dl:
                    ...
                    if _lego_file and _lego_file.exists():
                        ...
                        st.download_button("⬇️", _single_json.encode("utf-8"),
                                           file_name=f"{_lego_fname or f'item_{i+1}'}.json",
                ...
                if not _df_gap.empty:
                    ...
                    if _cat_analysis_data["品牌"]:
                        ...
                        for cat35, stats in _cat_analysis_data["品牌"].items():
                            ...
                            alr = stats["link_rate_sum"] / _total if _total > 0 else 0
                            ...
                            link_coverage = hl * 100 / _total if _total > 0 else 0
                            ...
                            brand_rows.append({"类别": cat35, "短语数": _total, "品牌/产品名称提及率": f"{bmr:.1f}%", "官方链接覆盖率": f"{link_coverage:.1f}%", "平均链接率(7平台)": f"{alr:.1f}%", "_lk": alr})
                    ...
                    if _cat_analysis_data["行业"]:
                        ...
                        for cat35, stats in _cat_analysis_data["行业"].items():
                            ...
                            link_coverage_i = hl * 100 / _total if _total > 0 else 0
                            ...
                            ind_rows.append({"类别": cat35, "短语数": _total, "品牌/产品名称提及率": f"{bmr:.1f}%", "官方链接覆盖率": f"{link_coverage_i:.1f}%", "平均链接率(7平台)": f"{alr:.1f}%", "_lk": alr})
